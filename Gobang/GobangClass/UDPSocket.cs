...
namespace GobangClass
{
    public partial class UDPSocket : Component
    {
        ...
        protected void Listener()   //监听
        {
            ...
            {
                int port = new Random().Next(localPort + 1, 65535);//随机生成端口号
                UDP_Server = new UdpClient(port);  //创建一个新的端口号
                MessageBox.Show(localPort + " 端口已经被占用，系统将自动为您分配端口号，自动分配的端口号为：" + port);
            }
        ...
        public void Send(System.Net.IPAddress Host, int Port, byte[] Data)
        {
            try
            {
                int pp = this.localPort;//用指定的IP地址和端口号初始化server
